...
typedef struct
{
	const RC_ctrl_t *shoot_rc;                   //ÈÅ•ÊéßÂô®ÊåáÈí?
  shoot_mode_e fric_mode;               //Â∞ÑÂáªÊéßÂà∂Áä∂ÊÄÅÊú∫
	shoot_mode_e last_fric_mode;          //Â∞ÑÂáª‰∏äÊ?°ÊéßÂà∂Áä∂ÊÄÅÊú∫
	fric_Motor_t motor_fric[2];          //Â∞ÑÂáªÁîµÊú∫Êï∞ÊçÆ
	fp32 fric_CAN_Set_Current[2];
	PidTypeDef motor_speed_pid[4];             //Â∞ÑÂáªÁîµÊú∫ÈÄüÂ∫¶pid
	
  first_order_filter_type_t fric1_cmd_slow_set_speed;  // Êª§Ê≥¢Êï∞ÊçÆ
  first_order_filter_type_t fric2_cmd_slow_set_speed;  // Êª§Ê≥¢Êï∞ÊçÆ
	fp32 angle[2];
	int16_t ecd_count[2];
