...
namespace SerialSpace
{
    ...
    //该共用体用于类型转换
    typedef union serialMessageSwitch    
    {
        float floatMessage;
        char charMessage[sizeof(float)];
    }serialMessageSwitch;
    ...
    private:
        ...
        //串口数据的帧头
        std::string serialMessageBegin;
        //共用体用于变量类型转换
        serialMessageSwitch messageSwitch;
    public:
        SerialMessage(void);
        //构造函数,默认帧头为0xFF 0X42
        SerialMessage(float* serialBegin, int size);

    public:
        //真实数据转为串口数据
        std::string messageToSerialMessage(std::string message);
        //串口数据转为真实数据
        std::string serialMessageToMessage(std::string serialMessage);
    };
