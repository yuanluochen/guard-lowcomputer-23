...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else
    {
        ...
        {
            // 鼠标按下右键设置为自瞄模式，该模式下云台运动受上位机控制
            gimbal_behaviour = GIMBAL_AUTO;
        }
        else //鼠标右键未按下设置未非自瞄模式，该模式下云台运动受用户控制
        {
            // 开关控制 云台状态
            if (switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[0]) && chassis_behaviour_mode != CHASSIS_NO_MOVE)
            {
...
static void gimbal_auto_control(fp32* yaw, fp32* pitch, gimbal_control_t* gimbal_control_set)
{
    ...
    if (gimbal_control_set->gimbal_vision_control->rx_flag)
    {
        ...
        //赋值移动
        *yaw = gimbal_control_set->gimbal_yaw_motor.gimbal_motor_kalman_filter.X_now;
        *pitch = gimbal_control_set->gimbal_pitch_motor.gimbal_motor_kalman_filter.X_now;
    }
    else
    {
        //未接收到上位机数据,不移动
